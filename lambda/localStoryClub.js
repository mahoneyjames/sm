...
async function createThemeChallenge(storage)
{
    const controller = require('./src/club/controllers/theme')(storage,storage);
    await controller.createThemeChallenge({themeText:'a nice little sausage',things:["1","2","3"]});
}

//createThemeChallenge(require('./src/club/storage/storage-local.js')({path:"_site/club/"}))

async function previewStory(storage,themeId, story)
{
    const controller = require('./src/club/controllers/theme')(storage,storage);
    await controller.previewStory(themeId, story);
}
